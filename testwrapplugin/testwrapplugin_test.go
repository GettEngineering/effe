...
func TestWrapPlugin(t *testing.T) {
	...
	strategy := strategies.NewChain(
		...
		strategies.Use(plugins.NewWrapError()),
	)
	...
	eTesting.RunTests(t, gen, "testdata", map[string][]byte{"github.com/GettEngineering/effe/testwrapplugin/gen.go": dslGo}, []string{"github.com/GettEngineering/effe/testwrapplugin"})
}
